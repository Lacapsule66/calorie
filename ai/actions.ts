...
export const alimentsSchema = z.array(
  z.object({
    aliment: z.string(),
    calories: z.number(),
    glucides: z.number(),
...
type Aliments = z.infer<typeof alimentsSchema>[];

export async function generateListeAliments({
  aliments,
}: {
  aliments: Aliments;
}) {
  const { object: listeAliments } = await generateObject({
    model: geminiFlashModel,

    prompt: `Si dans ${aliments} il y a deux aliment qui portent le meme nom met les que sur une entré et aditionne les valeurs, si pas d'aliments identique retourne simplement ${aliments} sans rien ajouter`,
    schema: alimentsSchema,
  });
...
export async function generateChoice({ question }: GenerateChoiceParams) {
  const { object: response } = await generateObject({
    model: geminiFlashModel,
    prompt: `${question} ,Génére une question qu'il s'adresse a l'user a manger quelque chose d'autre, cest question n'est pas pour toi donc ni répond pas !!! `,
    schema: z.object({
      question: z.string(),
    }),
  });
